...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4><i class="bi bi-plus-circle"></i> ثبت تسویه کالا</h4>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('inventory.settlements.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('settlement_reason_id') is-invalid @enderror"
                        ...
                        @foreach($settlementReasons as $reason)
                            <option value="{{ $reason->id }}" {{ old('settlement_reason_id') == $reason->id ? 'selected' : '' }}>
                                {{ $reason->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-check-lg"></i> ثبت تسویه
            </button>
        </form>
    </div>
</div>
