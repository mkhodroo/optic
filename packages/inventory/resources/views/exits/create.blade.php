...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4><i class="bi bi-plus-circle"></i> ثبت خروج کالا</h4>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('inventory.exits.store') }}" method="POST">
            @csrf
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('exit_reason_id') is-invalid @enderror"
                        ...
                        @foreach($exitReasons as $reason)
                            <option value="{{ $reason->id }}" {{ old('exit_reason_id') == $reason->id ? 'selected' : '' }}>
                                {{ $reason->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-check-lg"></i> ثبت خروج
            </button>
        </form>
    </div>
</div>
