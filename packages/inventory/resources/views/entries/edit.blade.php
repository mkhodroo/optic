...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4><i class="bi bi-pencil"></i> ویرایش ورود کالا</h4>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('inventory.entries.update', $entry) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('warehouse_id') is-invalid @enderror"
                        ...
                        @foreach($warehouses as $warehouse)
                            <option value="{{ $warehouse->id }}" {{ old('warehouse_id', $entry->warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                                {{ $warehouse->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control @error('quantity') is-invalid @enderror"
                        id="quantity" name="quantity" value="{{ old('quantity', $entry->quantity) }}" min="1" required>
                    @error('quantity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('entry_reason_id') is-invalid @enderror"
                        ...
                        @foreach($entryReasons as $reason)
                            <option value="{{ $reason->id }}" {{ old('entry_reason_id', $entry->entry_reason_id) == $reason->id ? 'selected' : '' }}>
                                {{ $reason->name }}
                            </option>
                        @endforeach
                    </select>
