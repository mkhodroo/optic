...
class IssueInvoice extends Controller
{
    private $case;
    ...
    public function execute(Request $request = null)
    {
        ...
        if(!$preInvoice){
            return response("اطلاعات پیش فاکتور یافت نشد. امکان صدور فاکتور وجود ندارد", 402);
        }
        ...
        $phpword = new TemplateProcessor(public_path( 'invoice_non_official.docx'));
        
        if($request->type == 'legal_with_added_value'){
            $phpword = new TemplateProcessor(public_path( 'official_invoice_with_added_value.docx'));
        }
        
        $phpword->setValue('date', $preInvoice->invoice_issued_date);
        ...
        $preInvociceFileName = "simpleWorkflow/فاکتور رسمی پرونده" . "-$case->number" . ".docx";
        $phpword->saveAs(public_path($preInvociceFileName));
        
        $preInvoice->invoice_file = $preInvociceFileName;
        $preInvoice->pre_invoice_type = $request->type;
        $preInvoice->save();
        return url('public/'. $preInvoice->invoice_file);
