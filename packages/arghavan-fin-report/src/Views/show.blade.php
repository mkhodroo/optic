...
@section('content')
        ...
        <div class="card-body row">
            @foreach ($case->repairCosts as $cost)
                ...
                <div class="col-sm-3">
                    ...
                    <p>[[ number_format($cost->cost) ?? $cost->cost ]]</p>
                </div>
            @endforeach
        </div>
    </div>
                ...
                <div class="col-sm-3">
                    ...
                    <p>
                        @if ($income->payment_receipt)
                            @if (str_contains($income->payment_receipt, 'http'))
                                <a href="[[ $income->payment_receipt ]]" download="">دانلود</a>
                            @else
                                <a href="[[ url('public/' . $income->payment_receipt) ]]" download="">دانلود</a>
                            @endif
                        @endif
                    </p>
                </div>
    ...
    @php
        $fieldName = 'مشاهده اطلاعات پیش فاکتور';
        $fieldDetails = getFieldDetailsByName($fieldName);
        $fieldValue = isset($case) ? $case->getVariable($fieldName) : null;
        $fieldValueAlt =
            (isset($case) and in_array($fieldDetails->type, ['datetime', 'date']))
