...
use Arghavan\FinReport\Models\Cases;
...
class MonthlyRequestsReportController extends AllRequestsReportController
{
    ...
    public function report(Request $request)
    {
        ...
        // کل پرونده ها
        $totalCases = (clone $itemsQuery)->count();

        // پرونده هایی که هیچ هزینه ای ندارند
        $casesWithoutCost = (clone $itemsQuery)
            ->doesntHave('repairIncomes')
            ->count();

        $casesWithoutCostPercent = $totalCases
            ? round(($casesWithoutCost / $totalCases) * 100, 2)
            : 0;
        ...
        $monthlyChart = [];
        $baseDate = Jalalian::fromFormat('Y-m-d', "{$year}-{$month}-01");
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $from = $date->getFirstDayOfMonth()->toCarbon();
            $to   = $date->getEndDayOfMonth()->toCarbon();

            $monthQuery = $this->baseQuery1()
                ->whereBetween('created_at', [$from, $to])
                ->whereNot('status', config('workflow.caseStatus.canceled.key'));

            $monthlyChart[] = [
                'month' => $date->format('%B'),
                'total' => $monthQuery->count(),
                'without_cost' => (clone $monthQuery)
                    ->doesntHave('repairIncomes')
                    ->count(),
            ];
        }

        return view('FinReport::MonthlyRequest.index')->with([
            'year' => $year,
            'month' => $month,
            'rows' => $rows,
            'total' => $rows->count(),
            ...
            'casesWithoutCost' => $casesWithoutCost,
            'casesWithoutCostPercent' => $casesWithoutCostPercent,
